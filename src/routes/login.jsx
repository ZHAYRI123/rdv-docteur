...
//hook React pour gérer les états locaux.
import { useState } from 'react';
//rediriger l’utilisateur après login/signup.
import { Link, useNavigate } from 'react-router-dom';
//pour afficher des notifications (ex: erreurs, succès).
import toast, { Toaster } from 'react-hot-toast';

//savoir si l'utilisateur est un médecin (true) ou un patient (false).
const Login = ({ isDoctor }) => {
	const [isLogin, setLogin] = useState(true);
	const [showpassword, setshowpassword] = useState(false);
    //utilisé pour rediriger vers une autre page après connexion ou inscription.
	...
	const handleLogin = async (event) => {
		...
		if (isDoctor) {
			try {
				const response = await fetch('http://localhost:5000/doctor/loginDoctor', {
					method: 'POST',
					headers: {
			...
			try {
				const response = await fetch('http://localhost:5000/patient/loginPatient', {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
					},
			...
			{isLogin ? (
				<div className='flex min-h-full flex-1 flex-col justify-center px-6 py-24 lg:px-8 '>
					<div className='sm:mx-auto sm:w-full sm:max-w-sm'>
						<img className='mx-auto h-10 w-auto' src='./logo.png' alt='Basmah Company' />
					...
					<div className='mt-10 sm:mx-auto sm:w-full sm:max-w-sm'>
						<form className='space-y-6' action='/' method='POST' id='loginForm'>
							...
							<div>
								<button type='submit' className='flex w-full justify-center rounded-md bg-green-600 px-3 py-1.5 text-sm font-semibold leading-6 text-white shadow-sm hover:bg-green-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600' onClick={handleLogin}>
									Log in
								</button>
							</div>
						</form>
				...
				<div className='flex min-h-full flex-1 flex-col justify-center px-6 py-24 lg:px-8'>
					...
					<div className='mt-10 sm:mx-auto sm:w-full sm:max-w-sm signupForm'>
						<form className='space-y-6' action='/' method='POST'>
							...
							<div>
								<button type='submit' className='flex w-full justify-center rounded-md bg-green-600 px-3 py-1.5 text-sm font-semibold leading-6 text-white shadow-sm hover:bg-green-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600' onClick={handleSignup}>
									Sign up
								</button>
							</div>
						</form>
