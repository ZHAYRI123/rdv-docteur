					...
					if (doctor.email === doctorEmail) {
						if (new Date(new Date(doctor.completionDate).getTime() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] < new Date().toISOString().split('T')[0]) {
							...
							setTimeout(async () => {
								...
								try {
									const updatePatientResponse = await fetch('http://localhost:5000/patient/updatePatient', {
										method: 'POST',
										headers: {
											'Content-Type': 'application/json',
											Authorization: `Bearer ${jwtToken}`,
									...
									try {
										...
										try {
											const updateDoctorResponse = await fetch('http://localhost:5000/doctor/updateDoctor', {
												method: 'POST',
												headers: {
													'Content-Type': 'application/json',
													Authorization: `Bearer ${jwtToken}`,
				...
				try {
					...
					if (updateDoctorResponse.status === 500) {
						...
						if (updateDoctor) {
							...
							try {
								const response = await fetch('http://localhost:5000/email/sendMail', {
									method: 'POST',
									headers: {
										'Content-Type': 'application/json',
										Authorization: `Bearer ${jwtToken}`,
